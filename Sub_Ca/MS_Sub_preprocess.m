function data_out = MS_Sub_preprocess(cfg_in, ms, behav, f_info)
%% MS_Sub_preprocess: preprocesses the spatial properties of a recording session.
...
for iC = 1:size(ms.Binary,2) % loop through cells
    ...
    if strcmpi(f_info.task, 'LT') || strcmpi(f_info.task, 'Linear Track')
        Place_Shuff_likelihood  = NaN(length(cfg.X_bins)-1, length(cfg.X_bins)-1,cfg.nShuff);
    else
    ...
    parfor iShuff   = 1:nShuf
        ...
        if strcmpi(task, 'LT') || strcmpi(task, 'Linear Track')
        ...
        [Acc_Shuff_MI(iShuff), ~, ~,~, Acc_Shuff_likelihood(:,iShuff)] = MS_get_spatial_information(shuffled_binarized, behav_aligned.accel(movement_idx(1:end-1)), cfg.a_bins);
    end
    toc
    fprintf('\n')
