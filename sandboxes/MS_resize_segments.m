...
switch cfg.tvec_to_use
    ...
    case 'NLX_csc'
        ...
        for iC = length(cfg.cutoffs):-1:1
            ...
            ms_seg_out.resize.org_times(iC) = (ms_seg_in.time{iC}(end) - ms_seg_in.time{iC}(1))./1000;
            ms_seg_out.resize.new_times(iC) = (ms_seg_in.time{iC}(tend_idx.time(iC)) - ms_seg_in.time{iC}(tstart_idx.time(iC)))./1000;
            ms_seg_out.resize.diff(iC) = ms_seg_out.resize.org_times(iC)-ms_seg_out.resize.new_times(iC);
            
            ms_seg_out.resize.org_nlx_times(iC) = ms_seg_in.NLX_csc{iC}.tvec(end) - ms_seg_in.NLX_csc{iC}.tvec(1);
            ms_seg_out.resize.new_nlx_times(iC) = tend_idx.nlx(iC) - tstart_idx.nlx(iC);
            ms_seg_out.resize.diff_nlx(iC) = ms_seg_out.resize.org_nlx_times(iC) - ms_seg_out.resize.new_nlx_times(iC);
