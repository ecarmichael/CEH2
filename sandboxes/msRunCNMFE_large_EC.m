...
if with_dendrites
    ...
    updateA_bSiz = 5;
    updateA_dist = neuron.options.dist; %this is not being passed EVV
else
    % determine the search locations by selecting a round area
    updateA_search_method = 'ellipse'; %#ok<UNRCH>
    updateA_dist = 5;
    updateA_bSiz = neuron.options.dist; %this is reversed? EVV
    updateA_bSiz = 5;
end
...
min_corr = 0.6;     % minimum local correlation for a seeding pixel, default 0.8
...
%% Reconstruct background EVV
if bg_model == 'ring'
    Background= neuron.reconstruct_background;
else 
    Background=neuron.f;
end
save([ms.dirName separator 'Background.mat'],'Background','-v7.3');

%Ybg = neuron.reconstructBG;
%[C_df,C_raw_df, Df] = extract_DF_F_endoscope(obj, Ybg);
end