function [out]  = MS_get_SCE_JC(ms, behav); 
...
ms_trk = msExtractBinary_detrendTraces(ms); 
...
% behav_i.time = behav_i.time+ms_trk.time(1); 
behav_i.position = []; behav_i.speed = []; 
behav_i.position(:,1) = interp1(behav_i.time, behav.position(:,1), ms_trk.time); 
behav_i.position(:,2) = interp1(behav_i.time, behav.position(:,2), ms_trk.time); 
% behav_i.speed = interp1(behav_i.time, behav.speed, ms_trk.time); 

behav_i.time = ms_trk.time; 


for ii = length(behav_i.time):-1:2
behav_i.speed(ii,:) = sqrt((behav_i.position(ii,1)-behav_i.position(ii-1,1)).^2 + (behav_i.position(ii,2)-behav_i.position(ii-1,2)).^2)/(behav_i.time(ii)/1000-behav_i.time(ii-1)/1000);
end
behav_i.speed(1,:) = 0; 
...
MS_Ca_Raster(data_og', ms_trk.time/1000);
% xlabel('frame number')
ylabel('cell id')
set(gca, 'xtick', [])
...
hold on
plot(ms_trk.time/1000, pop_act);
plot(ms_trk.time(SCE_idx)/1000, pop_act(SCE_idx), 'x')
...
plot(ms_trk.time/1000, behav_i.speed);
hline(7.5);
ylabel('speed (cm/s)')
set(gca, 'xtick', [])
...
hold on
plot(ms_trk.time/1000, behav_i.position(:,1));
plot(ms_trk.time(SCE_idx)/1000, behav_i.position(SCE_idx,1), 'xr')
...
hold on
plot(ms_trk.time/1000, behav_i.position(:,:));
