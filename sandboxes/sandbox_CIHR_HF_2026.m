...
log_iv.Tone = iv(data.vr.evt.t{ismember(data.vr.evt.label, 'Tone')},...
    data.vr.evt.t{ismember(data.vr.evt.label, 'Trace')}); 

log_iv.Trace = iv(data.vr.evt.t{ismember(data.vr.evt.label, 'Trace')},...
    data.vr.evt.t{ismember(data.vr.evt.label, 'Puff')}); 

log_iv.Puff = iv(data.vr.evt.t{ismember(data.vr.evt.label, 'Puff')},...
...
for ii = 1:length(log_iv.Tone.tstart)
    ...
    bl = restrict(vr_licks, log_iv.Tone.tstart(ii)-20, log_iv.Tone.tstart(ii));
    trials.base.licks(ii) = length(bl.t{1})./20;

    tl = restrict(vr_licks, log_iv.Tone.tstart(ii), log_iv.Tone.tend(ii));
...
for iS  = length(s_list):-1:1
    ...
    data.vr.pos.data(abs(wheel.data) > 10 ) = NaN;

    wheel.data = fillmissing(wheel.data, 'nearest');

    data.vr.pos = tsd(wheel.tvec, [wheel.data; diff([wheel.data wheel.data(end)])], {'encoder' 'movement'});
    ...
    for ii = 1:length(log_iv.Tone.tstart)
        ...
        bl = restrict(data.vr.pos, log_iv.Tone.tstart(ii)-20, log_iv.Tone.tstart(ii));
        trials.base.mov(ii) = sum(bl.data(3,:)) / length(bl.tvec);

        tl = restrict(data.vr.pos, log_iv.Tone.tstart(ii), log_iv.Tone.tend(ii));
        trials.tone.mov(ii) = sum(tl.data(3,:)) / length(tl.tvec);

        trl = restrict(data.vr.pos, log_iv.Trace.tstart(ii), log_iv.Trace.tend(ii));
        trials.trace.mov(ii) = sum(trl.data(3,:)) / length(trl.tvec);
    ...
    data_out{iS}.data.vr.pos = data.vr.pos;
    ...
    % data_wheel.(sess) = [data_wheel.(sess); data_out{iS}.data.vr.pos.data(3,:)]
end
...
% for ii = 1:
% D1_mov = mean(data_out(cond1_idx).data.vr.pos.data(3,:))
...
for iS =1:length(data_out)
    ...
    for ii = 1:length(data_out{iS}.log_iv.Tone.tstart)
        ...
        if contains(data_out{iS}.sess, 'Rec')
            this_r = restrict(data_out{iS}.data.vr.pos, data_out{iS}.log_iv.Tone.tstart(ii)-20, data_out{iS}.log_iv.Trace.tend(ii)+20);
            this_l = restrict(data_out{iS}.licks, data_out{iS}.log_iv.Tone.tstart(ii)-20, data_out{iS}.log_iv.Trace .tend(ii)+20);
        else
            this_r = restrict(data_out{iS}.data.vr.pos, data_out{iS}.log_iv.Tone.tstart(ii)-20, data_out{iS}.log_iv.Puff.tend(ii)+20);
...
for iS  = length(s_list):-1:1
    ...
    data.vr.pos = tsd(wheel.tvec, [wheel.data; spd], {'encoder' 'spd'});
    ...
    for ii = 1:length(log_iv.tone1.tstart)
        ...
        % same thing but movement
        bl = restrict(data.vr.pos, log_iv.tone1.tstart(ii)-20, log_iv.tone1.tstart(ii));
        trials.base.mov(ii) = (sum(bl.data(3,:)) / length(bl.tvec))*100;

        tl = restrict(data.vr.pos, log_iv.tone1.tstart(ii), log_iv.tone1.tend(ii));
        trials.tone.mov(ii) = (sum(tl.data(3,:)) / length(tl.tvec))*100;

        trl = restrict(data.vr.pos, log_iv.tone1.tend(ii), log_iv.tone1.tend(ii)+20);
    ...
    for ii = 1:length(log_iv.tone2.tstart)
        ...
        % same thing but movement
        bl = restrict(data.vr.pos, log_iv.tone2.tstart(ii)-20, log_iv.tone2.tstart(ii));
        trials.base2.mov(ii) = (sum(bl.data(3,:)) / length(bl.tvec))*100;

        tl = restrict(data.vr.pos, log_iv.tone2.tstart(ii), log_iv.tone2.tend(ii));
        trials.tone2.mov(ii) = (sum(tl.data(3,:)) / length(tl.tvec))*100;

        trl = restrict(data.vr.pos, log_iv.tone2.tend(ii), log_iv.tone2.tend(ii)+20);
    ...
    data_rec_out{iS}.data.vr.pos = data.vr.pos;
...
for iS =1%:length(data_rec_out)
    ...
    for ii = length(data_rec_out{iS}.log_iv.tone1.tstart):-1:1
        ...
        this_r = restrict(data_rec_out{iS}.data.vr.pos, data_rec_out{iS}.log_iv.tone1.tstart(ii)-20, data_rec_out{iS}.log_iv.tone1.tend(ii)+35);
...
title("CS+ | context 2")
    ...
    for ii = length(data_rec_out{iS}.log_iv.tone2.tstart):-1:1
        ...
        this_r = restrict(data_rec_out{iS}.data.vr.pos, data_rec_out{iS}.log_iv.tone2.tstart(ii)-20, data_rec_out{iS}.log_iv.tone2.tend(ii)+35);
