...
cfg_def.peak_threshold = 0.01; % number of Std above the mean of the diff(evts)
cfg_def.min_dist = 5; % distance (samples) between peaks
...
%                                 drawnow
                ...
                % start
                %find start value
                    temp_evt = restrict(evt, evt.t{cfg.t_chan}(idx(iRec)-cfg.start_search), evt.t{cfg.t_chan}(idx(iRec+1)));
                    
                    idx_start =find(diff(temp_evt.t{cfg.t_chan}(1:abs(cfg.start_search))) > mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol);
                    
                    if ~isempty(idx_start)
                        low_val =2;
                    end
%                     
%                     this_diff = diff(temp_evt.t{cfg.t_chan}); 
%                     
%                     if this_diff <  mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol
%                     figure(11)
%                     plot(this_diff)
%                     drawnow
...
%                     find(diff(temp_evt.t{cfg.t_chan}(1:end)) > mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol);
                    
                    if length(temp_evt.t{cfg.t_chan}) <= 20
                        idx_low =find(diff(temp_evt.t{cfg.t_chan}(1:end)) > mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol);
                    else
                        idx_low =find(diff(temp_evt.t{cfg.t_chan}(1:20)) > mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol);
                    end
                    %                     check  plot
%                     figure(11)
%                     title(['Low: ' num2str(low_val) '   High: ' num2str(high_val)]);
%                     plot(diff(temp_evt.t{cfg.t_chan}))
%                     ylim([0 mode(diff(temp_evt.t{cfg.t_chan}))+1])
%                     pause(.2)
%                     drawnow
                ...
                while ~isempty(idx_high)
                    ...
                    temp_evt = restrict(evt, evt.t{cfg.t_chan}(idx(iRec)-low_val), evt.t{cfg.t_chan}(idx(iRec+1)+high_val));
                    if length(temp_evt.t{cfg.t_chan}) <= 20
                        idx_high =find(diff(temp_evt.t{cfg.t_chan}(1:end)) > mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol);
                    else
                        idx_high =find(diff(temp_evt.t{cfg.t_chan}(end-20:end)) > mode(diff(temp_evt.t{cfg.t_chan}))*cfg.gitter_threshol);
                    end
%                     figure(11)
