...
for iA = 1:length(A_list)
 ...
 if contains(lower('PV1060'), lower(A_list{iA}))
     cfg.fc = {'CSC1.ncs', 'CSC8.ncs'}; % CSC1 is the emg and the other is the best LFP channel
 elseif contains(lower('PV1252'), lower(A_list{iA}))
    ...
    cfg_emg.threshold = 0;
    cfg_emg.f = [15 30]; 
    emg_data = FilterLFP(cfg_emg,emg);
    
    emg_thresh = 60;
    TD_ratio = 1.5; 
    out.hypno = MS_get_hypno(lfp, emg_data, [], emg_thresh, TD_ratio); 
