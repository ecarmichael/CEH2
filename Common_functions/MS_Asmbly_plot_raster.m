...
cla
hold on
scatter(A_in.behav.time(A_in.move_idx)/1000, A_in.behav.position((A_in.move_idx),1),ones(size(A_in.behav.time(A_in.move_idx))), A_in.behav.speed(A_in.move_idx))
...
leg_val = [];
for iA = plot_idx
    plot(A_in.wake_tvec, A_in.P_proj(iA,:), 'color', c_ord(find(iA == plot_idx),:))
...
%% save the figure
if ~isempty(fig_dir)
    saveas(gcf, [fig_dir filesep A_out.info.subject '_' A_out.info.session '_' strrep(num2str(A_out.info.bin), '.', 'p') 's_bin_wake_raster.png']);
end
