function Coord_out = MakeCoord_rad(tsd_in, varargin)
...
% ecarmichael May 2024; reworked for radial maze
...
fprintf('Draw a circle around the center. double click when done...\n')
roi = drawcircle('color', 'r', 'FaceAlpha', 0.4); 
wait(roi)

fprintf('Circle drawn\n')
scatter(roi.Center(1), roi.Center(2),50,'o',  'MarkerFaceColor', 'k')

% convert circle to polyshape
n = 5000; 
theta = (0:n-1)*(2*pi/n);
x = roi.Center(1) + roi.Radius*cos(theta);
y = roi.Center(2) + roi.Radius*sin(theta);
P = polyshape(x,y);
...
%     l_roi = drawline
    % get the nearest center circle point and mak
    X{ii}(2) = roi.Center(1); 
    Y{ii}(2) = roi.Center(2); 

%     int_x = interp1(0:1, X{ii}, 0:0.001:1); 
%     int_y = interp1(0:1, Y{ii}, 0:0.001:1); 

    X_v(1) = X{ii}(1); %int_x; 
    Y_v(1) = Y{ii}(1); %int_x; 
    
    
    this_V = nearestvertex(P, X{ii}(1), Y{ii}(1)); 
    
    X_v(2) = P.Vertices(this_V,1); 
    Y_v(2) = P.Vertices(this_V,2);

    plot(X{ii}, Y{ii}, '-', 'color', c_ord(ii,:));
    plot(X_v, Y_v, 'k' );
    
%         text(X{ii}(1), Y{ii}(1),[num2str(ii) 'Start'], 'color', c_ord(ii,:));
%             plot(P.Vertices(this_V,1),P.Vertices(this_V,2),'d')

    text(P.Vertices(this_V,1), P.Vertices(this_V,2),[num2str(ii) 'End'], 'color', c_ord(ii,:));
