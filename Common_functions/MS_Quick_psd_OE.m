function psd = MS_Quick_psd_OE(Chan_to_use, type, save_dir)
%% MS_Quick_psd_OE: generates PSDs for each .contiuous in the current directory specified in 'Chan_to_use' 
...
%       Chan_to_use = {'100_RhythmData-B_CH1.continuous, '100_RhythmData-B_CH2.continuous}. 
...
switch type
    case 'fast'
        ...
        for iSite = 1:length(csc.label)
            [psd.(csc.label{iSite}).pxx, psd.(csc.label{iSite}).f] = pwelch(csc.data(iSite,1:cut), hanning(cfg_psd.hann_win), cfg_psd.hann_win/2, cfg_psd.hann_win*2 , csc.cfg.hdr{1}.SamplingFrequency);
...
if length(csc.label) <= 8
    legend(strrep(csc.label(sorted_labels_idx), '_', ' '),  'location', 'southwest', 'orientation', 'vertical');

else
    legend(strrep(csc.label(sorted_labels_idx), '_', ' '),  'location', 'southwest', 'orientation', 'vertical', 'NumColumns', ceil(length(csc.label)/8));
