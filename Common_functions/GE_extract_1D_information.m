function [MI, posterior, occupancy_vector, prob_being_active, likelihood ] = GE_extract_1D_information(binarized_trace, interp_behav_vec, bin_vector, inclusion_vector)
...
for ii = 1:length(bin_vector)-1
    binarized_spatial_vector = 0*binarized_trace;
    position_idx = find(interp_behav_vec >= bin_vector(ii) & interp_behav_vec < bin_vector(ii+1));
    
    if ~isempty(position_idx)
        binarized_spatial_vector(position_idx)=1;
        occupancy_vector(ii) = length(position_idx)/length(binarized_trace);
