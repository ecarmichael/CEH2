...
cfg_def.method = 'spec'; % method to use.  Alternative is 'wavelet' WIP;
...
for iChan = length(cfg.lfp_chans):-1:1
    ...
    if strcmp(cfg.method, 'wavelet')
        %         %WIP must deal with memory limitations.
        %         fprintf('<strong>%s</strong>: Preprocessing wavelet. This may take some time and is limited by avilable memory\n', mfilename);
        %         [WT, F] = cwt(data(cfg.lfp_chans(iChan),1:(12000*(1/cfg.Fs))), 1/cfg.Fs,'FrequencyLimits',[1 16]);
        %
        %         fb = cwtfilterbank('SignalLength', length(data(cfg.lfp_chans(iChan),:)),...
        %             'SamplingFrequency', 1/cfg.Fs, ...
        %             'FrequencyLimits',[2 12]);
        %
        %         [cfs,frq] = wt(fb,data(cfg.lfp_chans(iChan),:));
        %         t = (0:length(data(cfg.lfp_chans(iChan),:)) - 1)/(1/cfg.Fs);
        %         figure;
        %         imagesc(t(1:100*(1/cfg.Fs)),frq,abs(cfs(:,1:100*(1/cfg.Fs))));
        %         axis xy
        %         set(gca,'yscale','log');shading interp;axis tight;
        %         title('Scalogram');xlabel('Time (s)');ylabel('Frequency (Hz)')
        
    else
        % lower freq spectrogram.
...
if ~isempty(score_in)
    if isempty(cfg.spec.freq_high)
        Spec_subs = 2:length(cfg.lfp_chans):((length(cfg.lfp_chans)*2)+1);
    else
        Spec_subs = 2:length(cfg.lfp_chans):((length(cfg.lfp_chans)*2)*(length(cfg.lfp_chans)*2)+1);
    end
else
    if isempty(cfg.spec.freq_high)
        Spec_subs = 1:length(cfg.lfp_chans):((length(cfg.lfp_chans)*2));
    else
...
for iSub = 1:length(cfg.lfp_chans)
    if ~isempty(cfg.spec.freq_high)
        ...
        ax(iSub+100) = subplot(nSubplots, 1, Spec_subs(iSub):Spec_subs(iSub)+1);
        h = imagesc(T{iSub}, F{iSub}, 10*log10(P{iSub}));
        ylim([F{iSub}(1) F{iSub}(end)])
        axis xy
    end
...
while ishandle(h)
    ...
    if ismember(key_hit, cfg.state_keys(1:end-2))
        ...
        x_lim = xlim;
        fprintf('     %.0f   %.0f    %0.2f%%\n', x_lim(1), x_lim(2), round(x_lim(2)/(tvec(end)-tvec(1)),4)*100)
        if done == 1
            disp('Exiting')
            break
        end
    ...
    if ~isempty(emg)
        ax(999) = subplot(nSubplots, 1, nSubplots);
        plot(tvec(this_idx(1):this_idx(2)) - tvec(1), emg(cfg.emg_chan,this_idx(1):this_idx(2)), 'b',tvec(this_idx(1):this_idx(2)) - tvec(1), emg_rms(this_idx(1):this_idx(2)), '--r')
        ylim(cfg.emg_range)
        xlim([tvec(this_idx(1))- tvec(1), tvec(this_idx(2))- tvec(1)]);
        %     text(tvec(1)- tvec(1), cfg.emg_range(2)*.8, 'EMG')
        drawnow;
