...
cfg_def.bad_idx = []; % idx to remove for Nans or know bad sections. 
cfg_def.verbose = 1;
...
if ~isempty(cfg.bad_idx)
    fprintf('<strong>%s</strong>: cfg.bad_idx indicies replaced with NaN. Using NaN zscore...\n', mfilename)
    temp_data(cfg.bad_idx) = NaN;
end
...
switch cfg.method
    case 'zscore'
        if isempty(cfg.bad_idx)
            ...
            mu = nanmean(temp_data);
            sigma = nanstd(temp_data);
            temp_data = (temp_data - nanmean(temp_data))/nanstd(temp_data);
        end
        switch cfg.dcn
            case '>' 
...
% put the raw threshold in the cfg. 
cfg.raw_threshold = thr_out; 
