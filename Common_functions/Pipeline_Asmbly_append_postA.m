function A_in = Pipeline_Asmbly_append_postA(A_in, opts)
...
for ii = 1:length(A_in)
    if size(A_in{ii}.REM_Post_data, 1) < size(A_in{ii}.REM_Post_data, 2)
        ...
        REM_temp_all = assembly_patterns(A_in{ii}.REM_Post_data', A_in{ii}.info.opts);


        if isempty(REM_temp_all)
            fprintf('%.0f Assemblies found in Post REM (%.2fs binsize)\n',size(REM_temp_all,2), A_in{ii}.info.bin)
            ...
            A_in{ii}.postREM_stats= [];
            A_in{ii}.postREM_shuff.data = [];
            A_in{ii}.postREM_shuff.proj = [];
...
for iB = length(A_in):-1:1
    if isempty(A_in{iB}.postREM_A_pos)
...
%% get the locations of pre assembly activity in wake. 
win_s = 2;
for iB = length(A_in):-1:1
    if isfield(A_in{ii}, 'postREM_stats') && ~isempty(A_in{ii}.postREM_stats)
        thresh = A_in{ii}.postREM_stats.R_thresh;
        ...
        A_in{ii}.postREM_stats.R_thresh = [];
    end
end
