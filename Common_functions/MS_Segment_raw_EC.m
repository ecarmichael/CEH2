function ms_seg_resize = MS_Segment_raw_EC(cfg_in, csc_dir, ms_dir, ms_resize_dir)
...
%% align and remove the maze segment

maze_idx = find(contains(ms_seg.file_names, 'MAZE'));
fprintf('MS time samples %.0f   |    %.0f NLX', length(ms_seg.RawTraces{maze_idx}), length(evt_blocks{maze_idx}.t{cfg.evt.t_chan}))

% keep the track ms struct and save
keep_idx = 1:size(ms_seg.RawTraces,1);
keep_idx =keep_idx((keep_idx ~= maze_idx));
...
ms_seg = MS_remove_data_sandbox(cfg_rem, ms_seg, maze_idx);
...
ms_seg.removed{end+1} = TS_name{maze_idx};
...
hypno_labels(maze_idx) = [];

time_labels(maze_idx) = [];

evt_blocks(maze_idx)= [];
evt_duration(maze_idx) = [];
...
all_tvec_pre = []; all_tvec_post = [];
all_tvec_pre_REM = []; all_tvec_post_REM = [];
all_tvec_pre_SW = []; all_tvec_post_SW = [];
...
for iSeg = 1:length(ms_seg_resize.RawTraces)
    ...
    if strcmp(ms_seg_resize.pre_post{iSeg}, 'pre')
        all_tvec_pre = [all_tvec_pre; ms_seg.time];
        ...
        if strcmp(ms_seg_resize.hypno_label{iSeg}, 'REM')
            all_tvec_pre_REM = [all_tvec_pre_REM; ms_seg.time];
    ...
    elseif strcmp(ms_seg_resize.pre_post{iSeg}, 'post')
        all_tvec_post = [all_tvec_post; ms_seg.time];
...
% pre only
save([ms_resize_dir filesep 'all_tvec_pre.mat'], 'all_tvec_pre', '-v7.3');
...
% post only
save([ms_resize_dir filesep 'all_tvec_post.mat' ], 'all_tvec_post', '-v7.3');
...
% pre REM only
save([ms_resize_dir filesep 'all_tvec_pre_REM.mat'], 'all_tvec_pre_REM', '-v7.3');
...
% post SW only
save([ms_resize_dir filesep 'all_tvec_post_REM.mat' ], 'all_tvec_post_REM', '-v7.3');
...
% pre REM only
save([ms_resize_dir filesep 'all_tvec_pre_SW.mat'], 'all_tvec_pre_SW', '-v7.3');
...
% post SW only
save([ms_resize_dir filesep 'all_tvec_post_SW.mat' ], 'all_tvec_post_SW', '-v7.3');
