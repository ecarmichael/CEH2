...
% EC 2016-09-28 fixed an issue where an empty IV in the first few IVs would
% lead to the .usr values not being the same length as the IVs out. (Ex: 64
% IVs in, but first two do not have complete cycles --> 62 .usr values out)
...
for iIV = nIV:-1:1
    ...
    if isempty(this_pk_idx)
        flag(iIV) = 1;
        iv_in.usr.var_raw(iIV) = 0;
        iv_in.usr.nCycles(iIV) = 0;
        iv_in.usr.mean_filt(iIV) = 0;
        iv_in.usr.min_filt(iIV) = 0;
        iv_in.usr.var(iIV) = 0;
        continue
    else
