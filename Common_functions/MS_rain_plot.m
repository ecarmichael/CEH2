function [ax, p, stats_out] =  MS_rain_plot(data_in, group_idx, color, stats, x_vals)
...
% a_n = (-a)*scale_val; 
% a_n = (a_n./max(a_n))+.25; 
ax(ii).h{1} = patch(b,((a./max(a))./2)+x_vals(ii), color(ii,:));
...
% mean line
ax(ii).h{2} = line([Q(3) Q(3)],[-0.1 0.1] + x_vals(ii),'col','k','LineWidth',2);

ax(ii).box = rectangle('Position',[Q(1) x_vals(ii)-.1 Q(2)-Q(1) .2],'EdgeColor',[0 0 0 .75], 'LineWidth',2);
...
% whiskers
% ax(ii).h{6} = line([Q(5) Q(5)], [-0.05 0.05] + x_vals(ii),'col',[0 0 0 .5],'LineWidth',2);
% ax(ii).h{7} = line([Q(4) Q(4)],[-0.05 0.05] + x_vals(ii),'col',[0 0 0  .5],'LineWidth',2);
ax(ii).eb{1} = line([Q(4) Q(1)], [x_vals(ii) x_vals(ii)],'col',[0 0 0  .5],'LineWidth',2);
ax(ii).eb{2} = line([Q(2) Q(5)], [x_vals(ii) x_vals(ii)],'col',[0 0 0  .5],'LineWidth',2);
