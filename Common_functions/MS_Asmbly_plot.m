...
if isempty(map_out)  % only plot the stems
    ...
    for ii = 1: size(A_cells,2)
        ...
        view(90,90)
        text(20, .425, ['A. ' num2str(ii)], 'color', c_ord(ii,:), 'fontsize', 12)
        
        ylim([-0.1 0.5])
        xlabel({'cell ID' ; [num2str(length(A_cells{ii})) ' members']})
        xlim([0 length(A_temp(:,ii))])
        
    end
else   % switch to plotting with the
    ...
    for ii = 1: size(A_cells,2)
        ...
        if   c_ii+1 > l
            ...
            if ~isempty(fig_dir)
                ...
                print(gcf,  [fig_dir filesep info.subject '_' info.session '_Wake_A_summery_' num2str(f_n+1) '_' strrep(num2str(info.bin), '.', 'p') 's_bin.pdf'], '-dpdf','-r0')
