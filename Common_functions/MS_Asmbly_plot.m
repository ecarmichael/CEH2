...
for ii = 1: size(A_cells,2)
    ...
    cla
    hold on
    [N_C, edges] = histcounts(map_out{ii}.cent, length(map_out{ii}.bins(1):5:map_out{ii}.bins(end)));
   ...
   legend('Centroid', 'Location', 'northeast', 'Box', 'off')
    ...
    hold on
    scatter(1:size(map_out{ii}.cent,1), map_out{ii}.cent(sort_idx),55, 'ow');
    scatter(1:size(map_out{ii}.cent,1), map_out{ii}.cent(sort_idx),55, 'xw')
